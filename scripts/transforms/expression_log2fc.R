...
#
# Usage: Rscript expression_log2fc.R <samples_fn> <bed_fn> <output_dir>/
#
...
#   Columns: featureID, treatment, sample, dose, y (log2FC vs dose=0 controls)
...
for (s in series_list) {
    ...
    control_means <- dat %>%
        filter(dose == 0) %>%
        group_by(featureID) %>%
        summarise(mean_control_logCPM = mean(logCPM, na.rm = TRUE), .groups = "drop")

    dat %>%
        inner_join(control_means, by = "featureID") %>%
        mutate(y = logCPM - mean_control_logCPM) %>%
